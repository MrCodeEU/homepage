...
func TestSortProjects(t *testing.T) {
	projects := []Project{
		{Name: "no-priority-low-stars", Stars: 1},
		{Name: "no-priority-high-stars", Stars: 50},
		{Name: "high-priority", Stars: 5, Priority: 10},
		{Name: "low-priority", Stars: 100, Priority: 1},
		{Name: "same-stars-b", Stars: 10},
		{Name: "same-stars-a", Stars: 10},
	}

	// Use the same sort logic as Scrape()
	sortProjects(projects)

	expected := []string{
		"high-priority",        // priority 10
		"low-priority",         // priority 1
		"no-priority-high-stars", // priority 0, 50 stars
		"same-stars-a",         // priority 0, 10 stars, name "a" < "b"
		"same-stars-b",         // priority 0, 10 stars, name "b"
		"no-priority-low-stars",  // priority 0, 1 star
	}

	if len(projects) != len(expected) {
		t.Fatalf("Expected %d projects, got %d", len(expected), len(projects))
	}

	for i, name := range expected {
		if projects[i].Name != name {
			t.Errorf("Position %d: expected %q, got %q", i, name, projects[i].Name)
		}
	}
}

func TestPortfolioMetadata_Priority(t *testing.T) {
	// With priority set
	jsonData := `{"priority": 5, "featured": true}`
	var meta PortfolioMetadata
	if err := json.Unmarshal([]byte(jsonData), &meta); err != nil {
		t.Fatalf("Failed to unmarshal: %v", err)
	}
	if meta.Priority == nil || *meta.Priority != 5 {
		t.Errorf("Expected priority 5, got %v", meta.Priority)
	}

	// Without priority (should be nil)
	jsonData = `{"featured": true}`
	var meta2 PortfolioMetadata
	if err := json.Unmarshal([]byte(jsonData), &meta2); err != nil {
		t.Fatalf("Failed to unmarshal: %v", err)
	}
	if meta2.Priority != nil {
		t.Errorf("Expected nil priority, got %v", *meta2.Priority)
	}

	// Priority set to 0 (should be distinguishable from nil)
	jsonData = `{"priority": 0}`
	var meta3 PortfolioMetadata
	if err := json.Unmarshal([]byte(jsonData), &meta3); err != nil {
		t.Fatalf("Failed to unmarshal: %v", err)
	}
	if meta3.Priority == nil {
		t.Error("Expected non-nil priority for explicit 0")
	} else if *meta3.Priority != 0 {
		t.Errorf("Expected priority 0, got %d", *meta3.Priority)
	}
}
