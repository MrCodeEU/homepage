...
export interface StravaDiscipline {
	type: string;           // "cycling" | "training"
	label: string;          // "Cycling & Spinning" | "Training"
	count: number;
	total_time: number;     // seconds
	total_distance: number; // meters
	avg_heartrate: number;  // 0 if unavailable
